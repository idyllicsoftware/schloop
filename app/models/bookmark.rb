...
class Bookmark < ActiveRecord::Base
  ...
  def self.index(teacher, grade_id, subject_id, topic_id, options = {})
    ...
    liked_bookmarks_group_by_id = liked_bookmarks.group_by(&:sc_trackable_id)
    ...
    bookmarks.each do |bookmark|
      ...
      bookmark_formatted_data[:likes] = likes
      bookmark_formatted_data[:is_liked] = is_liked
      bookmark_formatted_data[:is_collaborated] = collaborated_bookmark_ids.include?(bookmark.id)
      bookmark_formatted_data[:is_followedup] = followed_bookmark_ids.include?(bookmark.id)
  ...
  def generate_crawl_data
    begin
      preview_object = LinkThumbnailer.generate(data)
  ...
  def track_bookmark(event, like_state, user)
    errors = []
    begin
      if (event.eql? 'like') && (like_state.eql? 'false')
    ...
    { success: errors.blank?, errors: errors, bookmark: id }
  end
