...
RSpec.describe Followup, type: :model do
  before(:each) do
    ...
    @student_profiles = StudentProfile.create!(student_id:@student.id, grade_id:@grade.id, division_id:171, status:"active")
  end
  it "should return result order in reverse of time of sharing to parents" do
    records = Followup.index_for_web(Student.first)
    count = records.count
    for i in 0..(count-2)
